...
public class Hex {
	...
	private static int solve(char[][] maze) {
		...
		if (bflag) {
			...
			for (Location e : bpath) {
				...
				flag2[e.x][e.y] = true;
				if (!dfs(maze, flag2, new ArrayList<Location>(), true)) { return 2; }
			}
			return 0;
		} else {
			if (diff == -1) return 0;
			for (Location e : rpath) {
				boolean[][] flag2 = new boolean[len][len];
				flag2[e.x][e.y] = true;
				if (!dfs(maze, flag2, new ArrayList<Location>(), false)) { return 1; }
			}
			return 0;
		}
	}

	private static boolean dfs(char[][] maze, boolean[][] flag, List<Location> rpath, boolean b) {
		for (int i = 0; i < maze.length; i++) {
			if ((b && !flag[i][0] && maze[i][0] == 'B') || (!b && !flag[0][i] && maze[0][i] == 'R')) {
				rpath.clear();
				if (dfs(maze, b ? new Location(i, 0) : new Location(0, i), flag, rpath, b)) return true;
			}
		}
		return false;
	}
	...
	private static boolean dfs(char[][] maze, Location loc, boolean[][] flag, List<Location> rpath, boolean b) {
