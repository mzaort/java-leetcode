...
public class Meetandparty2 {
	...
	private static long[] minDistance(Point[] array) {
		...
		sx[0] = xcor[0].x;
		for(int i = 1; i < len; i++) sx[i] = sx[i - 1] + xcor[i].x;
		long[] sy = new long[len];
		sy[0] = ycor[0].y;
		for(int i = 1; i < len; i++) sy[i] = sy[i - 1] + ycor[i].y;
		
		
		long[] res = {0, 0, Long.MAX_VALUE};
		for(int i = 0; i < len; i++){
			int j = binarySearch(ycor, xcor[i]);
			long val = sx[len - 1] - 2 * sx[i] + ((long)(2 * i - len + 2)) * xcor[i].x
					+ sy[len - 1] - 2 * sy[j] + ((long)(2 * j - len + 2)) * ycor[j].y;
