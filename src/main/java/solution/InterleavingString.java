...
public class InterleavingString {
	public static void main(String[] args) {
		...
		// System.out.println(is.isInterleave2("bbbbbabbbbabaababaaaabbababbaaabbabbaaabaaaaababbbababbbbbabbbbababbabaabababbbaabababababbbaaababaa",
	...
	public boolean isInterleave2(String s1, String s2, String s3) {
		...
		for (int i = 1; i <= len1; i++)
			hit[i][0] = hit[i - 1][0] && s1.charAt(i - 1) == s3.charAt(i - 1);
		for (int j = 1; j <= len2; j++)
			hit[0][j] = hit[0][j - 1] && s2.charAt(j - 1) == s3.charAt(j - 1);

		for (int i = 1; i <= len1; i++) {
			for (int j = 1; j <= len2; j++) {
				hit[i][j] = (hit[i - 1][j] && s1.charAt(i - 1) == s3.charAt(i + j - 1))
						|| (hit[i][j - 1] && s2.charAt(j - 1) == s3.charAt(i + j - 1));
			}
		}
