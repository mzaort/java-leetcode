	...
	public boolean exist(char[][] board, String word) {
		for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board[i].length; j++) {
				if (exist(new HashSet<Position>(), board, new Position(i, j), word, 0)) return true;
			}
		}
		return false;
	}

	private boolean exist(Set<Position> set, char[][] board, Position pos, String word, int cur) {
