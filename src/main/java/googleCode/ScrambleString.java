...
public class ScrambleString {
	...
	private boolean isScramble(Boolean[][][][] flag, String s1, int i1, int j1,
		...
		if(flag[i1][j1][i2][j2] == null){
			if(i1 == j1){
				...
				int i = i1;
				for(; i < j1 && !((isScramble(flag, s1, i1, i, s2, i2, i2 + i - i1) && isScramble(flag, s1, i + 1, j1, s2, i2 + i - i1 + 1, j2)) 
						|| (isScramble(flag, s1, i1, i, s2, j2 - i + i1, j2) && isScramble(flag, s1, i + 1, j1, s2, i2, j2 - i + i1 - 1))); i++);
	...
	private boolean f(Boolean[][][] v, int i, int j, int len,
			String s1, String s2) {
		if(v[i][j][len] == null){
