...
public class InterleavingString {
	...
	public boolean isInterleave2(String s1, String s2, String s3){
		...
		f[0][0] = true;
		for(int i = 1; i <= len1; i++) f[i][0] = f[i - 1][0] && s1.charAt(i - 1) == s3.charAt(i - 1);
		for(int j = 1; j <= len2; j++) f[0][j] = f[0][j - 1] && s2.charAt(j - 1) == s3.charAt(j - 1);
		for(int i = 1; i <= len1; i++){
			for (int j = 1; j <= len2; j++) {
				char c1 = s1.charAt(i - 1), c2 = s2.charAt(j - 1), c3 = s3
						.charAt(i + j - 1);
