...
public class LongestPalindromicSubstring {
    ...
    public String longestPalindrome(String s) {
        ...
        for(int i = 0; i < s.length(); i++) f[i][i] = true;
        for(int i = 0, j = 1; j < s.length(); i++, j++) f[i][j] = s.charAt(i) == s.charAt(j);
...
//    			f[i] = 0;
//    		}else{
//    			f[i] = f[2 * center - i] > right - i ? right - i : f[2 * center - i];
//    		}
			if (i <= right)
				...
				f[i] = 0;
    		while(str.charAt(i + f[i] + 1) == str.charAt(i - f[i] - 1)) f[i]++;
    		if(i + f[i] > right){
    			right = i + f[i];
    			center = i;
    		}
    	...
    	int imax = 1;
    	for(int i = 1; i < f.length - 1; i++) if(f[imax] < f[i]) {imax = i;}
