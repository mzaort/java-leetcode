...
public class ZigZagConversion {
    ...
    public String convert(String s, int nRows) {
    	...
    	int step = 2 * nRows - 2;
    	for(int i = 0; i < s.length(); i += step) sb.append(s.charAt(i));
    	for(int i = 1; i < nRows - 1; i++){
    		int frm1 = i, frm2 = step - i;
    		for(; frm2 < s.length(); frm1 += step, frm2 += step) sb.append(s.charAt(frm1)).append(s.charAt(frm2));
    		if(frm1 < s.length()) sb.append(s.charAt(frm1));
    	}
    	for(int i = nRows - 1; i < s.length(); i += step) sb.append(s.charAt(i));
