...
public class LetterCombinationsofaPhoneNumber {
	...
	private List<String> letterCombinations(char[][] table, String digits,
			int length) {
	...
	private void letterCombinations2(List<String> res, char[][] table,
			String tmp, String digits, int cur) {
		if (cur == digits.length()) {
			res.add(tmp);
		} else {
			int index = digits.charAt(cur) - '0';
			for (int i = 0; i < table[index].length; i++)
				letterCombinations2(res, table, tmp + table[index][i], digits,
						cur + 1);
		}
	}
