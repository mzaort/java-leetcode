...
public class RestoreIPAddresses {
    		...
    		for(int j = i + 1; j <= s.length() - 2 && isValid(s, i, j); j++){
    			for(int k = j + 1; k <= s.length() - 1 && isValid(s, j, k); k++){
    				if(isValid(s, k, s.length())){
    					res.add(s.substring(0, i) + "." + s.substring(i, j) + "." + s.substring(j, k) + "." + s.substring(k));
    				}
    			}
    		}
    	}
	...
	private boolean isValid(String s, int frm, int end) {
		switch(end - frm){
		case 1: return true;
		case 2: return s.charAt(frm) != '0';
		case 3:	return s.charAt(frm) != '0' && Integer.parseInt(s.substring(frm, end)) < 256;
	...
	private void restoreIpAddresses(List<String> res, String str, String s,
			int cur, int split) {
		if(split == 1){
