...
public class SubstringwithConcatenationofAllWords {
	...
	private boolean dfs(String s, int i, int len, Map<String, Integer> maploc) {
	...
	public List<Integer> findSubstring3(String S, String[] L) {
		...
		for (String e : L)
			expected.put(e, (val = expected.get(e)) == null ? 1 : val + 1);
		...
		for (int i = 0, alen = L.length * L[0].length(), step = L[0].length(), limit = S.length()
				- alen; i <= limit; i++) {
			int j = i;
