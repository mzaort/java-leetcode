...
public class SolutionA {
		...
		for (int i = 0; i < num; i++) {
			if ((str = next(series, i)) != null) {
				...
				if (!match) {
					...
					if (!res.equals(str)) { return error; }
				}
			}
		}
		return res;
	...
	private String next(String[] series, int cur) {
		...
		for (int k = 0; k < num && k < series.length; k++) {
			for (int i = 0; i < bit; i++)
				if (series[k].charAt(i) == '1') st[i] = 1;
		}
		for (int i = series.length - 1, j = (cur + 1) % num, cnt = 0; cnt < num
				&& i >= 0; i--, j = (j + 1) % num, cnt++) {
			for (int k = 0; k < bit; k++) {
				if (st[k] == 0) {
	...
	static String[] mask = { "1111110", "0110000", "1101101", "1111001", "0110011", "1011011", "1011111", "1110000",
			"1111111", "1111011" };
	...
	private boolean isValid(String[] series, Boolean[] flag, int val) {
		...
		for (int i = 0, cnt = 0; i < series.length && cnt < num; i++, cnt++) {
			for (int j = 0; j < bit; j++)
				if (series[i].charAt(j) == '1') flag[j] = true;
		}
		for (int i = (val + 1) % num, j = series.length - 1, cnt = 0; cnt < num
				&& j >= 0; i = (i + 1) % num, j--, cnt++) {
			for (int k = 0; k < bit; k++) {
				if (flag[k] == null) {
